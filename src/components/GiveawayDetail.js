...
import {
	contractGetGiveaway,
	contractGetParticipants,
	contractGetWinners,
} from '../near/near'

import { prettyTruncate } from '../utils'
...
function GiveawayDetail() {
	...
	const getGiveaway = async () => {
		let _giveaway
		try {
			_giveaway = await contractGetGiveaway({
				id: giveawayId,
			})
			if (!_giveaway) {
				setIsNotFound(true)
		...
		>
			{isNotFound && (
				<div className="h-screen flex items-center justify-center">
					<h4 className="text-3xl font-medium">Giveaway not found</h4>
				</div>
			)}
			{isLoading ? (
				<div className="h-screen flex items-center justify-center">
					<h4 className="text-3xl font-medium">Loading</h4>
				</div>
			) : (
						...
						<>
							...
							<div className="mt-8 flex m-auto space-x-4">
								...
								<div className="flex-1 border-2 border-gray-800 border-dashed rounded-md p-4">
									...
									<h2 className="mt-1 text-xl font-medium">
										{new Date(parseInt(giveaway.endDate)).toDateString()}
									</h2>
									<h2 className="text-xl font-medium">
										{new Date(parseInt(giveaway.endDate)).toLocaleTimeString()}
									</h2>
								</div>
							</div>
